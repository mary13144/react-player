...
const useDrag = (
    ...
    const dragData = useRef<DragState['dragData']>({
        startX: 0,
        startY: 0,
        endX: 0,
        endY: 0,
    ...
    useEffect(() => {
        if (!mouseDownEle || !mouseMoveEle || !mouseUpEle) return;
        if (!isMobile) {
            ...
            mouseDownEle.addEventListener('touchstart', handleMouseDown, {
                passive: true,
            });
            mouseUpEle.addEventListener('touchend', handleMouseUp, {
                passive: true,
            });
            mouseMoveEle.addEventListener('touchmove', handleMouseMove, {
                passive: true,
            });
        }
