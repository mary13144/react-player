...
const Progress: FC = memo(() => {
    // 播放器上下文
    const { videoAttributes, videoState, videoOption, videoMethod } =
    ...
    useDrag(
        {
            onDrag: (dragData) => {
                ...
                const percentX =
                    (dragData.endX - left) / progressBgRef.current.clientWidth;
    ...
    const handleProgressMove = useCallback(
        throttle((e: React.MouseEvent) => {
            const leftDistance =
                progressBgRef.current.getBoundingClientRect().left;
            const deltaX = e.clientX - leftDistance;
            setPositionX(deltaX);
        }, 10),
        [progressBgRef.current],
        ...
        >
            <div className={styles.progressBg} ref={progressBgRef}>
                <div
                    ...
                    style={{ width: calcBufferedPercent }}
                ></div>
                <div
                    className={styles.progressPlayed}
                    style={{
                        width: calcPlayedPercent,
                        backgroundColor: videoOption?.theme
                            ? videoOption.theme
                            : defaultTheme,
                    }}
                >
                    ...
                    <>
                        ...
                        >
                            <div
                                ...
                                style={{
                                    borderTop: `0.25rem solid ${videoOption?.theme ? videoOption.theme : defaultTheme}`,
                                }}
                            ></div>
                            <div
                                className={styles.tooltipBottom}
                                style={{
                                    borderBottom: `0.25rem solid ${videoOption?.theme ? videoOption.theme : defaultTheme}`,
                                }}
                            ></div>
                        </div>
                        <div
                            ...
                            style={{ left: `${PositionX}px` }}
                        >
                            {videoOption?.setProgressTimeTip ? (
                                videoOption.setProgressTimeTip(
                                    transToMinutesAndSeconds(
                                        calcCurrentTime(PositionX),
                                    ),
                                )
                            ) : (
