...
const Controller = memo(function () {
    const { videoMethod, videoOption, videoAttributes, videoState, dispatch } =
    ...
    const handleMaskMove = useCallback(
        throttle(() => {
    ...
    useEffect(() => {
        if (isEnded) dispatch?.({ type: 'isControl', data: false });
    }, [isEnded]);
    ...
    const handleTouched = () => {
        ...
        if (
            timeSinceLastTouchEnd < doubleClickDelay.current &&
            timeSinceLastTouchEnd > 0
        ) {
            // 在这里添加双击事件的逻辑
            changePlayState?.();
    ...
    return (
        ...
        >
            ...
            {isShowProgressFloat !== false && !isControl && (
                <SuspendedProgress theme={theme || defaultTheme} />
            )}
            {isEnded &&
                (setEndContent ? (
                    setEndContent
