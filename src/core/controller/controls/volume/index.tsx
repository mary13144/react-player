...
const Volume = memo((props: VolumeProps) => {
    ...
    const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const volumePercent =
            (volumeRef.current!.getBoundingClientRect().bottom - e.clientY) /
            height;
        if (volumePercent > 1) {
    ...
    useDrag(
        {
            onDrag: (dragData) => {
                const bottom =
                    volumeRef.current!.getBoundingClientRect().bottom;
