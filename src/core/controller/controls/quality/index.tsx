		...
		isShowToast !== false && showToast({
			message: (
				<div>
					...
					<strong style={{color: theme}}>{language === 'zh' ? curQuality.zhName : curQuality.enName}</strong>
				</div>
			),
						...
						<ul className={styles.qualitySelectList}>
							{
								qualityList.map(item => {
									return (
										<li
											key={item.key}
											onClick={() => handleChangeQuality(item.key)}
											className={styles.qualitySelectItem}
											style={{color: item.key === currentKey ? theme : 'rgba(256, 256, 256, 0.7)'}}
										>
