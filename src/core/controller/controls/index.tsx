...
const Controls = memo(function () {
    const { videoOption, videoELe, videoContainerEle, videoState } =
    ...
    const inPictureKey = useMemo(
        () => (!isInPicture ? 'openPicture' : 'closePicture'),
        [isInPicture],
    );

    // 网页全屏
    const [isFullWebScreen, setIsFullWebScreen] = useState<boolean>(false);
    const isFullWebScreenKey = useMemo(
        () => (!isFullWebScreen ? 'FullWebScreen' : 'CloseWebFullScreen'),
        [isFullWebScreen],
    );
    const isFullWebScreenIcon = useMemo(
        () => (!isFullWebScreen ? 'webFullscreen' : 'closeWebFullscreen'),
        [isFullWebScreen],
    );

    // 全屏
    const [isFullScreen, setIsFullScreen] = useState<boolean>(false);
    const isFullScreenKey = useMemo(
        () => (!isFullScreen ? 'FullScreen' : 'CloseFullScreen'),
        [isFullScreen],
    );
    const isFullScreenIcon = useMemo(
        () => (!isFullScreen ? 'fullScreen' : 'closeFullScreen'),
        [isFullScreen],
    );
            ...
            <div className={styles.multifunctionContainer}>
                {!isMobile && qualityConfig && (
                    <div className={styles.multifunctionItem}>
                        <Quality theme={curTheme} language={lang}></Quality>
                    </div>
                )}
